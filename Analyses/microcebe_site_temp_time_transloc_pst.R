...
modelName <- "microcebe_site_temp_time_transloc_pst"
...
nimModel <- nimbleCode({
  ...
  for(f in 1:n.fragments){
    for(s in 1:2){
      logit.phi0[f,s] ~ dnorm(0,0.01) ## Baseline survival[site,sex]
      beta.time[f,s] ~ dnorm(0,0.01)  ## Temporal effect[site,sex]
      beta.temp[f,s] ~ dnorm(0,0.01)  ## Temperature effect[site,sex]
      beta.transloc[f,s] ~ dnorm(0,0.01)  ## Translocation effect[site,sex]
      for(m in 1:n.months){
        logit(PHI[f,s,1,m]) <- logit.phi0[f,s] +
          beta.temp[f,s] * temp[m] + 
          beta.time[f,s] * month[m] 
        logit(PHI[f,s,2,m]) <- logit.phi0[f,s] +
          beta.temp[f,s] * temp[m] + 
          beta.time[f,s] * month[m] +
          beta.transloc[f,s]     
      }#m
  ...
  for(f in 1:n.fragments){
      for(t in 1:n.sessions[f]){
        for(s in 1:2){
      p[f,t,s] ~ dunif(0,1)    ## Detection prob
      }#s
    }#t
  }#f
  ...
  for(i in 1:n.individuals){
    for(t in 2:n.sessions[i]){
      y[i,t] ~ dbern(p[site[i],sessIndex[i,t],sex[i]] * z[i,t])
    }#t
  }#i
})
...
n.iter <- dim(nimMat)[1]
beta.temp <- array(nimMat[ ,5:8], c(n.iter,8,2))
beta.time <- array(nimMat[ ,9:12], c(n.iter,8,2))
beta.int <- array(nimMat[ ,1:4], c(n.iter,8,2))
beta.transloc <- array(nimMat[ ,13:16], c(n.iter,8,2))
logit.phi0 <- array(nimMat[ ,25:28], c(n.iter,8,2))
...
nimMat <- do.call(rbind, nimOutput)
n.iter <- dim(nimMat)[1]
gamma.temp <- array(nimMat[ ,17:20], c(n.iter,8,2))
lambda0 <- array(nimMat[ ,21:24], c(n.iter,8,2))
