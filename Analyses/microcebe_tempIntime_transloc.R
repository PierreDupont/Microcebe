...
modelName <- "microcebe_tempIntime_transloc"
...
nimModel <- nimbleCode({
  ...
  beta.int[1,1] ~ dnorm(0,0.01)   ## Interaction effect female/disturbed
  beta.int[2,1] ~ dnorm(0,0.01)   ## Interaction effect male/disturbed
  beta.int[1,2] ~ dnorm(0,0.01)   ## Interaction effect female/protected
  beta.int[2,2] ~ dnorm(0,0.01)   ## Interaction effect male/protected
  
  beta.transloc[1,1] ~ dnorm(0,0.01) ## Translocation effect female/disturbed
  beta.transloc[2,1] ~ dnorm(0,0.01) ## Translocation effect male/disturbed
  beta.transloc[1,2] ~ dnorm(0,0.01) ## Translocation effect female/protected
  beta.transloc[2,2] ~ dnorm(0,0.01) ## Translocation effect male/protected
  
  ## Monthly survival probabilities
  for(m in 1:n.months){
    logit(PHI[1,1,1,m]) <- logit.phi0[1,1] + beta.temp[1,1] * temp[m] + beta.time[1,1] * month[m] + beta.int[1,1] * temp[m] * month[m]
    logit(PHI[2,1,1,m]) <- logit.phi0[2,1] + beta.temp[2,1] * temp[m] + beta.time[2,1] * month[m] + beta.int[2,1] * temp[m] * month[m]
    logit(PHI[1,2,1,m]) <- logit.phi0[1,2] + beta.temp[1,2] * temp[m] + beta.time[1,2] * month[m] + beta.int[1,2] * temp[m] * month[m]
    logit(PHI[2,2,1,m]) <- logit.phi0[2,2] + beta.temp[2,2] * temp[m] + beta.time[2,2] * month[m] + beta.int[2,2] * temp[m] * month[m]
    logit(PHI[1,1,2,m]) <- logit(PHI[1,1,1,m]) + beta.transloc[1,1]
    logit(PHI[2,1,2,m]) <- logit(PHI[2,1,1,m]) + beta.transloc[2,1]
    logit(PHI[1,2,2,m]) <- logit(PHI[1,2,1,m]) + beta.transloc[1,2]
    logit(PHI[2,2,2,m]) <- logit(PHI[2,2,1,m]) + beta.transloc[2,2]
  }# months
...
n.iter <- dim(nimMat)[1]
beta.temp <- array(nimMat[ ,5:8], c(n.iter,2,2))
beta.time <- array(nimMat[ ,9:12], c(n.iter,2,2))
beta.int <- array(nimMat[ ,1:4], c(n.iter,2,2))
beta.transloc <- array(nimMat[ ,13:16], c(n.iter,2,2))
logit.phi0 <- array(nimMat[ ,25:28], c(n.iter,2,2))
    ...
    for(s in 1:2){
      for(t in 1:2){
        PHI[ ,s,d,t,m] <- ilogit(logit.phi0[ ,s,d] +
                                   ...
                                   beta.time[ ,s,d] * month[m] +
                                   beta.int[ ,s,d] * temp[m] * month[m] +
                                   beta.transloc[ ,s,d] * (t-1))
      }#t
    }#s
  }#d
}# months
...
nimMat <- do.call(rbind, nimOutput)
n.iter <- dim(nimMat)[1]
gamma.temp <- array(nimMat[ ,17:20], c(n.iter,2,2))
lambda0 <- array(nimMat[ ,21:24], c(n.iter,2,2))
