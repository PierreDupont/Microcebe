...
modelName <- "microcebe_site_temp_time_transloc"
...
nimModel <- nimbleCode({
  ...
  for(s in 1:n.sites){
    for(ss in 1:2){
        logit.phi0[s,ss] ~ dnorm(0,0.01) ## Baseline survival[site,sex]
        beta.time[s,ss] ~ dnorm(0,0.01)  ## Temporal effect[site,sex]
        beta.temp[s,ss] ~ dnorm(0,0.01)  ## Temperature effect[site,sex]
        beta.transloc[s,ss] ~ dnorm(0,0.01)  ## Translocation effect[site,sex]
        for(m in 1:n.months){
          logit(PHI[s,ss,1,m]) <- logit.phi0[s,ss] +
            beta.temp[s,ss] * temp[m] + 
            beta.time[s,ss] * month[m] 
          logit(PHI[s,ss,2,m]) <- logit.phi0[s,ss] +
            beta.temp[s,ss] * temp[m] + 
            beta.time[s,ss] * month[m] +
            beta.transloc[s,ss]     
  ...
  for(i in 1:n.individuals){
    ...
    for(t in 1:n.intervals[i]){
      phi[i,t] <- prod(PHI[site[i],sex[i],transloc[i],start.int[i,t]:end.int[i,t]])
  ...
  for(s in 1:n.sites){
    for(ss in 1:2){
      lambda0[s,ss] ~ dnorm(0,0.01)    ## Detection Hazard rate female/disturbed
      gamma.temp[s,ss] ~ dnorm(0,0.01) ## Detection Hazard rate female/disturbed
    }
  }
...
nimInits <- list( z = z.init,
                  logit.phi0 = matrix(2.5,8,2),
                  beta.time = matrix(0,8,2),
                  beta.temp = matrix(0,8,2),
                  beta.transloc = matrix(0,8,2),
...
n.iter <- dim(nimMat)[1]
beta.temp <- array(nimMat[ ,grep("beta.temp",dimnames(nimMat)[[2]])], c(n.iter,n.sites,2))
beta.time <- array(nimMat[ ,grep("beta.time",dimnames(nimMat)[[2]])], c(n.iter,n.sites,2))
beta.transloc <- array(nimMat[ ,grep("beta.transloc",dimnames(nimMat)[[2]])], c(n.iter,n.sites,2))
logit.phi0 <- array(nimMat[ ,grep("logit.phi0",dimnames(nimMat)[[2]])], c(n.iter,8,2))
PHI <- array(NA, c(n.iter,n.sites,2,2,n.months))
for(f in 1:n.sites){
  for(m in 1:n.months){
    for(s in 1:2){
      for(t in 1:2){
