...
modelName <- "microcebe_temp_transloc"
...
nimModel <- nimbleCode({
  ...
  for(m in 1:n.months){
    logit(PHI[1,1,1,m]) <- logit.phi0[1,1] + beta.temp[1,1] * temp[m] 
    logit(PHI[2,1,1,m]) <- logit.phi0[2,1] + beta.temp[2,1] * temp[m] 
    logit(PHI[1,2,1,m]) <- logit.phi0[1,2] + beta.temp[1,2] * temp[m] 
    logit(PHI[2,2,1,m]) <- logit.phi0[2,2] + beta.temp[2,2] * temp[m] 
...
nimParams <- c("logit.phi0", "beta.temp","beta.transloc",
               "gamma.temp", "lambda0")
...
beta.transloc <- nimMat[ ,5]
logit.phi0 <- array(nimMat[ ,14:17], c(n.iter,2,2))
...
nimMat <- do.call(rbind, nimOutput)
n.iter <- dim(nimMat)[1]
gamma.temp <- array(nimMat[ ,6:9], c(n.iter,2,2))
lambda0 <- array(nimMat[ ,10:13], c(n.iter,2,2))
